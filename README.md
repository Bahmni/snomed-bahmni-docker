...
This directory contains bahmni dockerization replated scripts and files. More details can be found in [this](https://bahmni.atlassian.net/wiki/spaces/BAH/pages/3132686337/SNOMED+FHIR+Terminology+Server+Integration+with+Bahmni) Wiki link
...
The steps to build the docker image for your implementation specific config can be found [here](https://github.com/Bahmni/default-config#docker-image-build). Once you have the docker image built you need to update `Bahmni Config Environment Variables` in the [.env](snomed-standard/.env) file.
...
1. Enable debugging by uncommenting & setting `OMRS_DEV_DEBUG_PORT` to some port (e.g 8000) in the [.env](snomed-standard/.env#L80) and [docker-compose](snomed-standard/docker-compose.yml#L152#L170) files
...
To set CATALINA_HOME and JAVA_OPTS or other Environment variables  used by the Tomcat startup scripts, set the value of [these variables](snomed-standard/docker-compose.yml#L153#L154), [here](snomed-standard/.env#L81#L82). The values of CATALINA_HOME and JAVA_OPTS are set as 
...
5. When you want to add a new module click on `Choose file` in the Add Module section and then select your .omod file.
6. When you want to add a new module click on `Choose file` in the Upgrade Module section and then select your .omod file.
7. Then click on Upload. OpenMRS will pick up the new module and will be available to use.
8. The modules folder has been volume mounted and will persist until you remove your volume.
